// RockmanPicConv.cpp
// MSVC6-adapted build of the decompiled RockmanPicConv tool.
// Converted to compile as C++ in MSVC6 with small portability fixes.
...
#ifndef uintptr_t
typedef unsigned long uintptr_t; /* MSVC6: pointer size is 32-bit */
#endif

/* ─────────────────────────────────────────────────────────────────
 *  GX enums and string tables
 * ───────────────────────────────────────────────────────────────── */
typedef enum {
    GX_TF_IA4     = 0,
    GX_TF_IA8     = 1,
...
typedef struct TgaImageBlock {
    ...
    unsigned int    dfPtr;          /* stores palette pointer as integer */
    ...
    unsigned char   colorMapType;   /* extra for convenience */
} TgaImageBlock;
...
/* TCReadTplTextFile implementation */
...
static int DecodeTgaFile(const char *filename, TgaImageBlock **outBlock)
{
    ...
    block->colorMapType    = hdr.colorMapType;
    ...
    assert(rawBits != NULL);
    if (!rawBits) { free(palData); free(block); fclose(fp); return 0; }
...
static void AdjustTgaForPaletteOffset(TgaImageBlock *block)
{
    if (block->paletteOffset == 0) return;
    unsigned int numPixels = (unsigned int)block->width * block->height;
    if (block->pixelDepth == 8) {
...
static int TCWriteTplFile(TplState *state, const char *outPcpPath)
{
    ...
    unsigned char cnt_be[4] = {
        (unsigned char)((imgCount >> 24) & 0xFF),
        (unsigned char)((imgCount >> 16) & 0xFF),
        (unsigned char)((imgCount >>  8) & 0xFF),
        (unsigned char)((imgCount >>  0) & 0xFF)
    };
...
static int WritePaletteBlockRGB565(FILE *fp, TplPaletteEntry *pal, int paletteIdx)
{
    ...
    if ((int)written != pal->numColors) {
        fprintf(stderr, "WritePaletteBlockRGB565:  error in fwrite for palette %d\n", paletteIdx);
        return -1;
    }
    return 0;
}
static int WritePaletteBlockRGB5A3(FILE *fp, TplPaletteEntry *pal, int paletteIdx)
{
    ...
    size_t written = fwrite(pal->palData, 2, (size_t)pal->numColors, fp);
    if ((int)written != pal->numColors) {
        fprintf(stderr, "WritePaletteBlockRGB5A3:  error in fwrite for palette %d\n", paletteIdx);
        return -1;
    }
    return 0;
}
